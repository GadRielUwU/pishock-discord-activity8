...
export const onRequest = async (context: { request: Request; env: Env }): Promise<Response> => {
  ...
  try {
    ...
    if (!state.overlimitEntitlementId) {
      return jsonResponse({
        success: false,
        error: 'No consumable entitlement available.',
      }, 403);
    }
    ...
    return jsonResponse({
      success: true,
      consumedEntitlementId: state.overlimitEntitlementId,
    });
  } catch (error) {
    return jsonResponse({
      success: false,
      error: error instanceof Error ? error.message : 'Failed to consume entitlement',
    }, 500);
  }
};
